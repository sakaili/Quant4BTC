...
class FactorSelector:
    ...
    def maybe_select(self, df_history: pd.DataFrame, *, bar_index: Optional[int] = None) -> float:
        """Select a factor using strictly historical data (no current bar leakage)."""
        df_history = df_history if df_history is not None else pd.DataFrame()
        cur_idx = int(bar_index if bar_index is not None else len(df_history))
        history_len = len(df_history)
        min_hist = self._history_requirement()
        force_recalc = bool(getattr(self.cfg, "force_factor_recalc", False))

        if history_len < min_hist:
            factor = self._fallback_factor_value()
            self._last_factor = factor
            self._selection_meta = {
                "method": "history_fallback",
                "factor": float(factor),
                "reason": "insufficient_history",
                "details": {"history_len": history_len, "required": min_hist},
                "fallback": True,
                "reuse": False,
            }
        ...
        if (not force_recalc) and (not self._allow_recalc(cur_idx)):
            self._mark_reuse()
            return self._ensure_last_factor()

        try:
            f = float(self._select(df_history))
        except Exception as exc:
            logger.warning("Factor selection failed, reuse last factor: %s", exc)
            f = self._ensure_last_factor()
            self._selection_meta = {
            ...
            last = self._ensure_last_factor()
            if abs(f - last) > self.cfg.factor_sticky:
                self._last_factor = f
