...
class SuperTrendStrategy(Strategy):
    ...
    def run_once(self, equity: float | None = None) -> None:
        ...
        if self._last_executed_signal is not None and trade_signal == self._last_executed_signal:
            signal_repeated = True
            self.logger.info("Signal %s unchanged, logging metrics without new orders", trade_signal)
        ...
        exec_signal = current_signal if not signal_repeated else 0
        ...
        stop_loss = self._risk_levels(last_close, st, exec_signal)
        target_contracts = self._compute_position_size(exec_signal, last_close, stop_loss, equity)

        desired_long = desired_short = 0
        if self.cfg.mode == "long_flat":
            desired_long = target_contracts if exec_signal == 1 else 0
        else:
            if exec_signal == 1:
